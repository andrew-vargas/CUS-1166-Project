...
public class Job {
	...
	public Job(String username, int jobID, int jobDuration, String jobName, String jobInfo) {
	...
	public class JobViewer
	{
		...
		public JobViewer() {
			...
			jobMenu.setBounds(300, 70, 155, 25);
			//(300, 110, 155, 25);
		...
		public class submitJobListener implements ActionListener {
			...
			{
				try {
					 result =Client.startClientServer();
				...
				Job newJob = new Job(userName, newJobID,newTimeMin,newJobName,newJobInfo);
				SQLDataConnection.insertJobSQL(newJob);
				
				/* List<Job> ownerList = FileReader.Job("SavedInfo\\JobInfo.txt"); <---- I couldn't get this to work
				for (Job jobs: ownerList) {
					String tempID = jobs.getJobID() + "";
					if (tempID.equals(inID.getText()))
					{
						System.out.println("Job ID already in use!");
						return;
					}
		 		} */
				...
				try {
					...
					fileWriter.println();
							// get information inputed into program and print into file
					fileWriter.print(userName+ ",");
					fileWriter.print(newJobName+ ",");
					fileWriter.print(newJobID+ ",");
				...
				savedMessage.setText("<html>The job has been submitted.</html>");
					...
					inMinutes.setText("");
					inInfo.setText("");
					savedMessage.setText("<html>The job has been rejected.</html>");
				}
			}		
		}
